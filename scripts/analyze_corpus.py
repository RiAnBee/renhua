...
ADVERSATIVE = (r"(?:^|[。！？；\n])\s*(?:然而|但是|可是|不过|尽管如此|反之)|"
               r"然而|但是|，不过|。不过")
# 并列/补充类连接词。**与转折连词不同，它在两个语域方向一致**（见 evidence.md A4）。
# 只取**句首**形态：`以及` 是短语内并列词（"A、B 以及 C"），不是句际衔接标记，
# 计入会抬高人类侧、把倍率从 5.78× 稀释到 2.63× —— 与本规则要处理的现象无关。
# 独立印证：Kong & Macken 2025 (MT Summit) 的 top-10 判别特征含 ratio_paraConj。
ADDITIVE = r"(?:^|[。！？；\n])\s*(?:同时|此外|另外|与此同时|除此之外|与此相关)"
